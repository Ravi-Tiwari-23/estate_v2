...
export default function Navbar() {
  ...
  return <header className="sticky top-0 z-50 border-b border-black/5 bg-cream/90 backdrop-blur-xl">
    <div className="mx-auto flex h-20 max-w-[1440px] items-center gap-8 px-5 lg:px-10">
      ...
      <nav className="hidden items-center gap-6 text-sm font-medium lg:flex">{nav.map(([label,href])=><Link className={active(href)?'text-green':'text-ink/65 hover:text-ink'} href={href} key={href}>{label}</Link>)}</nav>
    ...
    {open && <nav className="border-t border-black/5 bg-cream px-5 py-5 lg:hidden">{nav.map(([label,href])=><Link onClick={()=>setOpen(false)} className="block border-b border-black/5 py-3 font-medium" href={href} key={href}>{label}</Link>)}<div className="mt-5 grid grid-cols-2 gap-3"><Link className="btn-outline" href="/signin">Sign in</Link><Link className="btn-dark" href="/sell">List property</Link></div></nav>}
  </header>;
}
